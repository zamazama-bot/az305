...
},
{
  ...
  answer: 0,
  explanation: "可用性テスト(URL Pingテストやマルチステップテスト)は、世界中の複数のポイントから定期的に外部到達性を確認する合成監視で、一定割合の地点で失敗した場合にダウンタイムとして検知・アラートできます。スマート検出は失敗率の急上昇などテレメトリのパターンを自動分析する機能で、外部からの疎通確認ではありません。アプリケーションマップはコンポーネント間の依存関係を可視化するものです。ライブメトリックはアプリ内部の準リアルタイムテレメトリのストリーミングであり、外部からの可用性確認ではありません。"
},
{
  domain: "ID・ガバナンス・監視",
  type: "multi",
