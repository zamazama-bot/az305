...
},
{
  ...
  answer: 1,
  explanation: "Basic Logsはテーブルごとに選択できる低コストのログプランで、大量かつ低頻度アクセスの診断ログの取り込みコストを大幅に抑えられます(目安としてAnalytics Logsの取り込み単価の1/5程度)。トレードオフとして、<strong>クエリの実行自体はAnalytics Logsでは無料である一方、Basic Logsではクエリのたびにスキャンしたデータ量(GB)に応じた従量課金が発生</strong>し、利用できるKQLの機能も制限されます(複雑な結合が不可・アラート不可など)。既定の対話型保持期間も短くなります。「取り込み量は多いが検索頻度は低い」というこのシナリオでは、Basic Logsへの変更で取り込みコストの削減効果がクエリ課金の増加分を上回るため最適です。Analytics Logsはフル機能・フル価格のプランでコスト最適化には不向きです。アーカイブは取り込み後のデータ保持コストを下げる設定であり、取り込みコスト自体を下げるものではありません。Sentinelの分析ルールはテーブルプランとは無関係です。"
},
{
  domain: "ID・ガバナンス・監視",
