...
},
{
  ...
  answer: 0,
  explanation: "漏洩認証情報の一致（Leaked Credentials検出）は、その1回のサインイン試行そのものに紐づく<strong>サインインリスク</strong>として評価されます。Identity Protectionが提供する<strong>サインインリスクベースのConditional Accessポリシー</strong>を構成すると、Microsoftの機械学習・脅威インテリジェンスに基づいてリアルタイムに算出されたリスクレベル（低・中・高）に応じて、そのセッションだけをブロックしたり追加のMFAを要求したりできます。この機能はMicrosoft Entra ID <strong>P2</strong>ライセンスで利用可能です。<br><br>ユーザーリスクポリシーは、個々のサインインではなく<strong>アカウントそのもの</strong>が侵害されている可能性（漏洩情報の継続的悪用パターンや異常な行動の蓄積など）を評価する仕組みで、典型的にはパスワード変更を要求してアカウントを是正する用途に使われます。今回のような「そのサインイン試行を即座に止めたい」というリアルタイム性の高い要件には、サインインリスクポリシーの方が直接的に対応します。名前付きの場所によるIPブロックは静的なホワイトリスト／ブラックリスト方式であり、動的に更新される漏洩認証情報データベースとの照合という脅威インテリジェンスの仕組みは持ちません。Sentinelの分析ルールはログを収集した後にインシデントとして可視化・記録する検知後の対応であり、サインインの試行自体をその場でブロックすることはできません。",
},
{
  domain: "ID・ガバナンス・監視",
...
},
{
  ...
  answer: 0,
  explanation: "Azure Policyの<strong>Deny</strong>効果は、ポリシー条件（この場合はパブリックIPアドレスの割り当てを含むリクエスト）に一致するリソースの作成・更新要求を、Azure Resource Manager（ARM）のリクエスト処理パイプラインの段階でリアルタイムに拒否します。ユーザーがデプロイを試みた瞬間にエラーとして返され、リソースは一切作成されません。「作成の試み自体を拒否する」というプロアクティブな制御には、この効果が正確に対応します。<br><br>Appendは、リクエストの内容に指定したフィールド（タグや設定値など）を強制的に追加してからデプロイを続行させる効果であり、作成自体を止めるものではありません。AuditIfNotExistsは関連リソースの有無を評価し、条件を満たさない場合に対象を非準拠としてコンプライアンスダッシュボードに記録するだけの事後的な可視化機能で、作成をブロックする力は持ちません。Modifyは新規または既存リソースの特定プロパティを書き換える効果（例：タグの追加や削除）であり、これもリソースの作成自体を拒否する仕組みではありません。",
},
{
  domain: "ID・ガバナンス・監視",
...

{
  ...
  answer: 1,
  explanation: "<strong>Azure SQL Managed Instance</strong>はSQL Serverのデータベースエンジンをほぼそのままインスタンス単位でPaaS化したサービスで、クロスデータベースクエリ・トランザクション、リンクサーバー、SQL Server Agentジョブ、CDC、CLRなど、単一データベースでは提供されないインスタンススコープの機能を高い互換性で利用できます。パッチ適用・バックアップ・OSメンテナンスはAzureが自動的に行うため、VMでのセルフ管理に比べて運用負荷が大幅に下がります。<br><br><strong>Azure SQL Database（単一データベース）</strong>と<strong>エラスティックプール</strong>はデータベース単位の抽象化であり、そもそもインスタンスという概念が存在しないため、クロスデータベーストランザクションやリンクサーバー、SQL Server Agentは利用できません。<strong>SQL Server on Azure Virtual Machines</strong>はOSレベルまで含めて完全な互換性を持ちますが、パッチ適用やSQL Serverのインストール・アップグレードを自社で行う必要があり、「運用管理の負荷を最小化したい」という要件には合致しません。<br><br>なお、オンプレミスSQL Serverからの移行では、ダウンタイムをほぼゼロに抑えられる<strong>Managed Instance link</strong>（分散可用性グループに似た継続的なデータ同期機能）も用意されており、段階的な移行やハイブリッド構成にも対応できます。",
},
{
  domain: "データストレージ",
...
},
{
  ...
  answer: 1,
  explanation: "<strong>ライフサイクル管理ポリシー</strong>はストレージアカウントに定義するルールベースの自動化機能で、Blobの最終変更日、または（アクセス追跡を有効にした場合の）最終アクセス日を条件として、指定日数が経過したBlobを自動的にCoolやArchiveなどの下位アクセス層へ移動したり削除したりできます。JSON形式でルールを1度定義すれば以降は完全にマネージドで動作するため、運用担当者の手作業を排除できます。<br><br><strong>Azure Functions</strong>による独自実装は動作こそしますが、スケジュール実行の管理、Blob一覧の走査、エラーハンドリングなどを自前で構築・保守する必要があり、既にマネージドで提供されている機能を再発明することになります。<strong>Data Factory</strong>によるスキャンとパイプライン化も同様に不要な複雑さとコストを伴います。<strong>最初からArchiveティアで保存し手動で戻す</strong>方法は、Archiveからの復元（リハイドレート）に数時間から場合によっては1日以上かかる上、アクセス頻度に応じた自動的な最適化にもなっていません。<br><br>最終アクセス日基準のルールを使う場合は、ストレージアカウントで「アクセス時間の追跡（Last Access Time Tracking）」を別途有効化しておく必要がある点に注意してください。",
},
{
  domain: "データストレージ",
