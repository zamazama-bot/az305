...
},
{
  ...
  answer: 0,
  explanation: "Conditional Accessの<strong>認証強度（Authentication Strength）</strong>は、単に「MFAを満たしているか」ではなく「どの認証方式の組み合わせで満たしたか」まで細かく指定できる付与制御です。Microsoft提供の組み込み強度の1つである「<strong>フィッシング耐性のある多要素認証</strong>」を選択すると、FIDO2セキュリティキー、Windows Hello for Business、証明書ベース認証（スマートカードなど）といった、中間者攻撃・偽サイトによる資格情報窃取に強い方式のみが許可され、SMSやパスワード＋通知承認のような可傍受・フィッシング可能な方式は自動的に排除されます。特権ロール保有者に対する保護強化として、Microsoftのセキュリティベースラインでも推奨される構成です。<br><br>単純に「多要素認証を要求する」制御を設定するだけでは、SMSコードや音声通話、Microsoft Authenticatorの単純承認プッシュ通知なども条件を満たしてしまうため、フィッシング耐性という要件を満たしません。デバイス準拠の要求は、端末がIntuneなどで管理・準拠状態にあるかというデバイスの信頼性を評価するものであり、サインイン時に使う認証方式自体を制限するものではありません。アプリの強制制御（App Enforced Restrictions）はExchange OnlineやSharePoint Online側でネイティブに提供されるセッション制限機能（例：非準拠デバイスでのダウンロード制限）であり、認証強度の指定とは異なる仕組みです。",
},
{
  domain: "ID・ガバナンス・監視",
...
},
{
  ...
  answer: 0,
  explanation: "Key Vaultはデータプレーンの権限モデルとして「<strong>Vaultアクセスポリシー</strong>」と「<strong>Azure RBAC</strong>」の2種類を排他的に持ちます。既定では前者が有効になっているため、PIMによる時限的なロールアクティブ化を含むAzure RBACベースの一元管理に切り替えるには、Key Vaultリソースの「アクセス構成（Access configuration）」設定を明示的に<strong>Azure ロールベースのアクセス制御</strong>へ変更する必要があります。この切り替えを行わない限り、RBACロール（Key Vault Secrets Userなど）を割り当てても評価されず、既存のアクセスポリシーの定義がそのまま優先され続けます。<br><br>アクセスポリシーの個々の権限（GetやListなど）をいくら調整しても、モデル自体がアクセスポリシー方式のままであれば、RBACロールへの移行という根本的な目的は達成されません。リソースロックはリソースの削除・変更操作を防ぐ機能であり、データプレーンの権限モデル（誰がシークレットを読めるか）とは無関係な設定です。ネットワーク設定（プライベートエンドポイント）は接続経路（どこから接続できるか）を制御するものであり、認可モデル（RBACかアクセスポリシーか）の切り替えとは別軸の話です。",
},
{
  domain: "ID・ガバナンス・監視",
...
},
{
  ...
  answer: 0,
  explanation: "<strong>AuditIfNotExists</strong>効果は、評価対象リソース（VM）に関連付けられているべき別のリソース（この場合は診断設定）が存在するかどうかをチェックし、存在しない場合に対象を「非準拠」としてマークし、Azure Policyのコンプライアンスダッシュボードに反映します。この効果はあくまで評価・記録のみを行い、実際のリソース作成や構成変更は一切行わないため、「自動修復は行わず、まず可視化だけを行いたい」という要件に正確に合致します。後日、必要になった時点で同等の条件を持つDeployIfNotExistsポリシーへ切り替えたり、修復タスクを個別に実行したりする段階的な運用がしやすい点も実務上のメリットです。<br><br>DeployIfNotExistsは、AuditIfNotExistsと似た条件評価を行いますが、非準拠と判定された場合にマネージドIDの権限を使って関連リソースを自動的にデプロイ（修復）してしまうため、「自動修復は行わない」という明示的な要件に反します。Denyはリソースの新規作成・更新要求自体をブロックする効果であり、既に存在する既存VMの事後的なコンプライアンス評価には使用できません（新規作成時のみ働く）。Modifyは対象リソース自身のプロパティを直接書き換える効果であり、診断設定のような別リソースの存在チェックや自動デプロイには使用しません。",
},
{
  domain: "ID・ガバナンス・監視",
...
},
{
  ...
  answer: 0,
  explanation: "<strong>Microsoft Entra External ID（旧称Azure AD B2C）</strong>は、不特定多数の一般消費者を対象とするカスタマー向けアイデンティティ基盤（CIAM）として設計されたサービスです。Google、Facebook、Appleなどの外部ソーシャルIDプロバイダーとのフェデレーションや、メールアドレス＋パスワードによるローカルアカウントでのサインアップ・サインインを標準でサポートし、<strong>ユーザーフロー</strong>（定型的な画面遷移）や<strong>カスタムポリシー</strong>（Identity Experience Frameworkによる高度なカスタマイズ）を使って、企業独自のブランドを反映したサインイン／サインアップ画面のUIを柔軟に構築できます。数百万規模のコンシューマーアカウントをスケーラブルに扱う前提で設計されている点も特徴です。<br><br>B2Bコラボレーションは、既に何らかの組織的なID（取引先企業のEntra IDアカウントなど）を持つビジネスパートナーとの連携を目的とした機能であり、組織に属さない一般消費者向けの認証基盤としては設計されていません。Entra Domain Servicesはオンプレミス互換のLDAP/Kerberosドメインサービスを提供するものであり、モバイルアプリの消費者向け認証とは全く異なる用途です。社内の従業員向けテナントへゲストとして一般消費者を登録する方法は、想定される利用者数の規模やセキュリティ境界の設計として適切ではなく、そもそもそのような大規模消費者向け用途は意図されていません。",
},
{
  domain: "ID・ガバナンス・監視",
...
},
{
  ...
  answer: 2,
  explanation: "<strong>Premiumティア</strong>は、Azure Cache for Redisの従来型（非Enterprise系）ティアの中で最上位に位置し、RDBスナップショットやAOF（Append Only File）によるディスクへの<strong>データ永続化</strong>、複数シャードにデータを分割する<strong>クラスタリング</strong>、そして仮想ネットワーク内への直接デプロイ（VNet統合）といった、エンタープライズ用途で求められる高度な機能をサポートします。<br><br><strong>Basicティア</strong>は単一ノード構成でSLAが提供されず、永続化機能もありません。<strong>Standardティア</strong>はプライマリ/レプリカ構成による自動フェイルオーバーとSLAを提供しますが、データ永続化・クラスタリング・VNet直接デプロイはいずれもサポートされていません。<br><br><strong>Enterpriseティア（Flashオプション含む）</strong>はRedis Enterprise由来の最上位シリーズで、RediSearchやRedisJSONなどのモジュール、アクティブ-アクティブ型の地理的レプリケーションといったPremiumにはない機能を持ちますが、本問が求める基本要件（永続化・クラスタリング・VNet統合）自体はPremiumティアで満たされるため、コスト効率の面でも標準的な選択肢はPremiumティアになります。Premiumのgeoレプリケーションは1次-2次（パッシブ）構成である点も、Enterpriseのアクティブ-アクティブ構成との違いとして押さえておくとよいでしょう。",
},
{
  domain: "データストレージ",
