...
},
{
  ...
  choices: [
    ...
    "Route Serverの「分岐間接続(Branch-to-branch)」設定を有効にする",
    "ピアリング済みのスポークVNetが、NVA経由のルートを動的に学習し利用する"
  ],
  answer: [0,1,2,3],
  explanation: "まずハブVNet内にAzure Route Serverをデプロイします(1)。次に、既存のNVAとAzure Route Serverの間でBGPピアリングを構成し、NVAが広告するルート情報をRoute Serverが受信できるようにします(2)。続けて、Route Serverの「分岐間接続(Branch-to-branch)」設定を有効にすることで、Route Serverが学習したルートをVNetピアリングされた他のVNet(スポーク)にも伝播できるようにします(3)。最終的に、ピアリング済みのスポークVNetは、UDRを個別に手動構成することなく、NVA経由の動的なルート情報を自動的に学習し、それに基づいて通信できるようになります(4)。"
},
{
  domain: "インフラ",
...
},
{
  ...
  answer: 0,
  explanation: "kubenetは、Node(VM)にのみVNetのIPアドレスを割り当て、Pod用のIPアドレスはNode内部でNATを使って別のアドレス空間から割り当てる、軽量なネットワークプラグインです。そのためVNetのIPアドレス消費を最小限に抑えられますが、機能やスケール(ルート数の上限など)に制約があり、Windowsノードプールもサポートしません。<br>・Azure CNI(従来)は、すべてのPodにVNet内の実アドレスを事前予約するため、IPアドレス消費が非常に大きくなります。<br>・Azure CNI Overlayもこの規模のクラスターには使えますが、kubenetの方がよりシンプルでこのシナリオの制約(小規模・Windows不使用)に合致します。<br>・Virtual Network NATはAKSのネットワークプラグインではなく、送信(アウトバウンド)接続のためのAzureサービスです。"
},
{
  domain: "インフラ",
