...
},
{
  ...
  answer: 3,
  explanation: "Azure CDNは、世界中に分散されたエッジPoP(Point of Presence)に静的コンテンツをキャッシュし、ユーザーに近い場所から低レイテンシで配信することに特化したサービスです。オリジンサーバーへのリクエスト数を減らし、負荷を軽減できます。WAFやパスベースルーティングのような高度な機能が不要な今回の要件には、最もシンプルでコスト効率の良い選択です。<br>Front DoorやApplication Gatewayはより高機能なレイヤー7サービスですが、静的コンテンツのキャッシュ配信のみが目的であれば過剰な機能です。Traffic ManagerはDNSルーティングのみでキャッシュ機能は持ちません。<br>なお、Azure CDN(classic)製品群は新規プロファイルの作成が停止されつつあり、Microsoftは今後の新規構築先として<strong>Azure Front Door Standard</strong>(CDN相当の機能を含む後継)への移行を案内しています。試験では現行のCDN製品として出題される可能性がありますが、実務での新規設計では原則Front Doorを検討してください。"
},
{
  domain: "ID・ガバナンス・監視",
  type: "multi",
  scenario: "Key Vaultの論理削除(Soft delete)と消去保護(Purge protection)の動作を確認しています。",
  question: "正しい説明を2つ選択してください。",
  choices: [
    "新規に作成されるKey Vaultでは、論理削除は既定で有効になっている",
    "消去保護(Purge protection)は一度有効にすると無効化できない",
    "消去保護を有効にしなくても、保持期間中の完全削除(パージ)を防止できる",
    "論理削除の保持期間はユーザーが1〜7日の範囲で任意に設定できる"
  ],
  answer: [0,1],
  explanation: "正しいのは1つ目と2つ目です。現在、新規に作成されるすべてのKey Vaultで論理削除は既定で有効になっており、無効化できません。削除されたオブジェクト(および削除されたVault自体)は保持期間中、削除済み状態で保持され、リストア(復元)が可能です。消去保護を有効にすると、この保持期間が終了するまでは、Owner権限を持つユーザーであっても完全な削除(パージ)を実行できなくなり、一度有効化すると無効化できない一方向の設定です。<br>3つ目は誤りです。消去保護を有効にしていない場合、削除権限を持つユーザーは保持期間中でもVaultやオブジェクトを完全にパージできてしまいます。悪意のある削除やランサムウェア攻撃からの保護を意図するなら、消去保護の有効化が必須です。4つ目も誤りです。論理削除の保持期間は既定90日で、7〜90日の範囲で設定します(1〜7日ではありません)。"
},
{
  domain: "インフラ",
...
},
{
  ...
  choices: [
    ...
    "既定で有効なAzure DDoSインフラストラクチャ保護(旧Basic)のみ",
    "Azure Bastionの導入",
...
{
  domain: "ID・ガバナンス・監視",
  scenario: "規制要件により、暗号キーをシングルテナント専用のHSMプールで保護する必要があるシステムを設計しています。<br>・キー管理はフルマネージドサービスを利用する<br>・他のテナントとHSMハードウェアを共有してはならない",
  question: "何を推奨しますか?",
  choices: [
  ...
  answer: 2,
  explanation: "Azure Key Vault Managed HSMは、フルマネージドでシングルテナント(自分専用)のFIPS 140-3 Level 3認定HSMプールを提供するサービスです。Key Vaultと同様のAPI/操作性を維持しながら、ハードウェアを他のテナントと共有しない専有プールでキーを保護できます。<br>Key VaultのStandardレベルはソフトウェアで保護されたキーのみをサポートし、HSM保護には対応しません。Premiumレベルもキーの保存にHSMを使用し、現在はManaged HSMと同じFIPS 140-3 Level 3認定のHSMを使用しますが、これはマルチテナント(複数顧客で共有)のHSMインフラであり、シングルテナント専有プールという要件は満たしません(PremiumとManaged HSMの本質的な違いは認定レベルではなく、マルチテナント共有かシングルテナント専有かという点です)。Azure Dedicated HSMは物理HSMアプライアンスをIaaS的に占有利用するサービスですが、Key Vault APIとの統合はなく、キー管理の運用をより多く自分で行う必要があるため、フルマネージドという要件には適しません。"
},
{
  domain: "インフラ",
  type: "multi",
